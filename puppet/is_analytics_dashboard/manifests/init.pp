...
class is_analytics_dashboard inherits is_analytics_dashboard::params {
  ...
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/":
    ensure => directory,
    owner  => $user,
    group  => $user_group,
  }
  ...
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/${is_package}":
    owner  => $user,
    group  => $user_group,
    mode   => '0644',
  ...
  exec { 'unzip':
    command => 'unzip wso2is-analytics-5.8.0.zip',
    unless =>  '/usr/bin/test -d /usr/lib/wso2/wso2is-analytics/5.8.0/wso2is-analytics-5.8.0',
    cwd     => '/usr/lib/wso2/wso2is-analytics/5.8.0/',
    path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
  }

  #jdk
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/${jdk_type}":
    owner  => $user,
    group  => $user_group,
    mode   => '0644',
  ...
  $template_list.each | String $template | {
    file { "/usr/lib/wso2/wso2is-analytics/5.8.0/wso2is-analytics-5.8.0/${template}":
      ensure  => file,
      owner   => $user,
      group   => $user_group,
      mode    => '0644',
  ...
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/wso2is-analytics-5.8.0/${start_script_template}":
    ensure  => file,
    owner   => $user,
    group   => $user_group,
    mode    => '0754',
  ...
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/wso2is-analytics-5.8.0/lib/${db_connector}":
    owner  => $user,
    group  => $user_group,
    mode   => '0754',
  ...
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/${jdk_path}/jre/lib/security/local_policy.jar":
    owner  => $user,
    group  => $user_group,
    mode   => '0754',
    ensure => present,
  ...
  file { "/usr/lib/wso2/wso2is-analytics/5.8.0/${jdk_path}/jre/lib/security/US_export_policy.jar":
    owner  => $user,
    group  => $user_group,
    mode   => '0754',
    ensure => present,
